      ...
      <div className="rounded-lg border border-slate-200 bg-surface p-4 shadow-sm">
        <ul className="space-y-2">
          {customers.map((customer) => (
            <li key={customer.id} className="flex items-center justify-between rounded-md border border-slate-100 px-3 py-2">
              <span className="font-medium text-slate-800">{customer.name}</span>
              <span className="text-sm text-slate-500">{customer.owner} · {customer.status}</span>
            </li>
          ))}
        </ul>
      </div>
