  ...
  providers: [
    Credentials({
      ...
      async authorize(credentials) {
        ...
        try {
          const data = await gqlMutation<{ login: { accessToken: string; user: { id: string; name: string; email: string } } }>(
          ...
          return { id: user.id, name: user.name, email: user.email, backendToken: accessToken } as any;
        } catch {
          return null;
        }
      }
  ...
  callbacks: {
    ...
    async jwt({ token, user, account }) {
      ...
      if (user && (user as any).backendToken) {
        ...
        token.id = user.id;
        token.roles = decodeJwtRoles((user as any).backendToken);
      }

      // OAuth login — sync user with backend
      if (account && account.provider !== 'credentials' && user?.email) {
        try {
          const data = await gqlMutation<{ loginWithOAuth: { accessToken: string; user: { id: string } } }>(
