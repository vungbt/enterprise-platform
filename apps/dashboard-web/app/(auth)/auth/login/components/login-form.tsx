...
import { Form, FormField, FormSubmit, Input, InputPassword, Button, Checkbox } from '@enterprise/ui/components';
import { useLogin, loginSchema } from '../utils/use-login';
  ...
  return (
    <div className="space-y-5">
      <Form form={form} schema={loginSchema}>
        <div className="space-y-4">
          <FormField name="email" label="Email" required>
            <Input type="email" placeholder="Enter your email" icon="envelope" autoComplete="email" />
          </FormField>
      ...
      <div className="space-y-3">
        <Button type="button" variant="outline" color="neutral" className='w-full' onClick={loginWithGoogle} disabled={oauthPending}>
          <GoogleIcon />
          <span className="text-neutral-black">Continue with Google</span>
        </Button>

        <Button type="button" variant="outline" color="neutral" className="w-full" onClick={loginWithMicrosoft} disabled={oauthPending}>
          <MicrosoftIcon />
...
function MicrosoftIcon() {
  return (
    <svg className="w-4 h-4 mr-2 shrink-0" viewBox="0 0 23 23">
